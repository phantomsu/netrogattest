(function () {
  ...
  // Загрузка плагина store.js
  function loadStorePlugin() {
      return new Promise((resolve, reject) => {
          const script = document.createElement('script');
          script.src = 'https://skaztv.online/store.js';
          script.onload = () => resolve(window.store); // Предполагается, что плагин добавляет глобальный объект `store`
  ...
  loadStorePlugin()
      .then(store => {
          storePlugin = store;
          console.log('Плагин store.js успешно загружен');
      })
      .catch(err => {
          console.error('Ошибка загрузки плагина store.js:', err);
      });
    ...
    function filmix(component, _object) {
      ...
      function skaztv(component, _object) {
        var network = new Lampa.Reguest();
        var extract = {};
        var object = _object;
        var select_title = '';
        ...
        this.search = function (_object, data) {
            ...
            if (storePlugin) {
                // Используйте методы плагина для поиска контента
                storePlugin.search(object.movie.title, function(results) {
                ...
                component.empty(Lampa.Lang.translate('online_nolink'));
            }
        };
    
        // Остальные методы (filter, reset, destroy и т.д.) аналогичны другим источникам
        this.reset = function() { /* ... */ };
        this.filter = function(type, a, b) { /* ... */ };
        this.destroy = function() { /* ... */ };
    ...
    function component(object) {
      ...
      var filter_sources = ['videocdn', 'rezka', 'kinobase', 'collaps', 'filmix', 'skaztv'];
